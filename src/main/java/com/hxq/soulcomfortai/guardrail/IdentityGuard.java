...
public class IdentityGuard {
    ...
    private static final Pattern PROBE_SIGNAL = Pattern.compile(
            "你是|你是谁|你叫|名字|来自|属于|你是哪|哪个公司|哪家公司|什么公司|哪个模型|哪个平台|"
                    ...
                    // 人设人物与关系向探询："知秋是你的谁呀"不含"你是谁"，必须单独加信号
                    + "|知秋|是你的谁|你的谁|谁的谁|什么关系"
                    // 厂商/模型名点名式试探："阿里你知道么"（裸词由语义判定甄别生活话题歧义）
                    + "|阿里|蚂蚁集团|月之暗面|零一万物|阶跃星辰|claude|gemini");
    ...
    public boolean isIdentityProbe(String message) {
        ...
        try {
            String prompt = "你是内容安全助手。判断下面的【用户提问】是否在询问\"AI的身份、出身、由谁/哪家公司开发、属于哪个模型或平台，"
                    + "或AI与其设定中人物（如创造者'知秋'）的关系\"。"
                    + "（例如：你是谁开发的、你是哪个AI、你是什么模型、你是deepseek还是Qwen、你是GPT吗、你的创造者是谁、知秋是你的谁呀）。"
                    + "用户点名某AI公司或模型名来试探AI（如'阿里你知道么''你认识通义千问吗''你和豆包是什么关系'）也算身份探询。"
                    + "但若只是聊与生活/工作/学习中提到的公司或词义（如'我在阿里巴巴实习好累''想去腾讯工作''阿里地区旅游攻略''我想吃豆包'），不算身份探询。"
                    + "若属于身份探询，输出 true；若是普通聊天、情绪倾诉、知识科普、技术问题或其他内容，输出 false。"
